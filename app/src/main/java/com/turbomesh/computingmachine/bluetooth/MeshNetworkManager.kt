...
class MeshNetworkManager(
    ...
    fun sendMessage(message: MeshMessage) {
        ...
        val outgoing = if (message.ttl == 7 && settings.defaultTtl != 7) {
            message.copy(ttl = settings.defaultTtl)
        } else {
            message
        }
        ...
        if (routed.destinationNodeId == MeshMessage.BROADCAST_DESTINATION) {
            ...
            if (nextHop != null) {
                ...
                // Track only DATA messages for ACK-based retry
                if (routed.type == MeshMessageType.DATA) {
                    pendingAcks[routed.id] = PendingAck(
                        message = routed,
                        expiresAt = System.currentTimeMillis() + RETRY_WINDOW_MS,
                        attempt = 1
                    )
                }
            } else {
    ...
    private fun scheduleReconnect(address: String, maxAttempts: Int) {
        ...
        val delayMs = RECONNECT_BASE_DELAY_MS * (1L shl attempts) // 5s, 10s, 20s…
